...
#define get_sector_offset(data, index) (\
    (uint32_t)(unsigned char)((data) + (index) + LOCATION_HEADER_OFFSET)[0] << (2 * 8) |\
    (uint32_t)(unsigned char)((data) + (index) + LOCATION_HEADER_OFFSET)[1] << (1 * 8) |\
    (uint32_t)(unsigned char)((data) + (index) + LOCATION_HEADER_OFFSET)[2] << (0 * 8) )\

#define get_sector_count(data, index) (\
    (uint8_t)(unsigned char)((data) + (index) + LOCATION_HEADER_OFFSET)[3])

#define get_mtime(data, index) (\
    (uint32_t)(unsigned char)((data) + (index) + MTIME_HEADER_OFFSET)[0] << (3 * 8) |\
    (uint32_t)(unsigned char)((data) + (index) + MTIME_HEADER_OFFSET)[1] << (2 * 8) |\
    (uint32_t)(unsigned char)((data) + (index) + MTIME_HEADER_OFFSET)[2] << (1 * 8) |\
    (uint32_t)(unsigned char)((data) + (index) + MTIME_HEADER_OFFSET)[3] << (0 * 8) )\

#define set_sector_offset(data, index, offset)\
    ((data) + (index) + LOCATION_HEADER_OFFSET)[0] = (char)(uint32_t)(offset) >> (2 * 8);\
    ((data) + (index) + LOCATION_HEADER_OFFSET)[1] = (char)(uint32_t)(offset) >> (1 * 8);\
    ((data) + (index) + LOCATION_HEADER_OFFSET)[2] = (char)(uint32_t)(offset) >> (0 * 8);

#define set_sector_count(data, index, count) \
    ((data) + (index) + LOCATION_HEADER_OFFSET)[3] = (char)(count);

#define set_mtime(data, index, mtime)\
    ((data) + (index) + MTIME_HEADER_OFFSET)[0] = (char)(uint32_t)(mtime) >> (3 * 8);\
    ((data) + (index) + MTIME_HEADER_OFFSET)[1] = (char)(uint32_t)(mtime) >> (2 * 8);\
    ((data) + (index) + MTIME_HEADER_OFFSET)[2] = (char)(uint32_t)(mtime) >> (1 * 8);\
    ((data) + (index) + MTIME_HEADER_OFFSET)[3] = (char)(uint32_t)(mtime) >> (0 * 8);
 ...
 */
static anvil_result handle_malformed(struct anvil_file *region_file) {
    ...
    if (msync(file, size, MS_INVALIDATE)) {
        __anvil_errno_return(munmap(file, size))
    }
...
static uint16_t *get_indices(struct anvil_file *region_file) {
...
try_chunk_name_again:
    const int path_len = anvil_get_name(
...
#error not implemented
    ...
    __anvil_assert_return(region_file_out != nullptr);
    ...
    *region_file_out = nullptr;
    ...
    struct anvil_file *region_file = alloc->malloc(sizeof(struct anvil_file));
    ...
    const auto name_len = anvil_get_name(nullptr, 0, "r", region_x, region_z, region_extension);
    ...
    anvil_get_name(region_file->tmp_string, region_file->tmp_string_cap, "r", region_x, region_z, region_extension);
...
anvil_result anvil_read(
    ...
    __anvil_assert_return(file != nullptr);
    __anvil_assert_return(file->file != nullptr || file->size == 0);
    ...
    __anvil_assert_return(file->size >= HEADER_SECTORS * SECTOR_SIZE);
    ...
    char *cursor = file->file + sector_offset * SECTOR_SIZE;
    ...
    if (separate_file) {
        ...
        if (close(fd)) {
            __anvil_errno_return(munmap(cursor, st.st_size));
        }
...
static anvil_result realloc_region_file(
    ...
    __anvil_assert_return(region_file != nullptr);
    __anvil_assert_return(region_file->file != nullptr || region_file->size == 0);
    __anvil_assert_return(sectors == 0 || sectors >= HEADER_SECTORS);
...
static anvil_result chunk_realloc(
    ...
    __anvil_assert_return(region_file != nullptr);
    __anvil_assert_return(region_file->file != nullptr || region_file->size == 0);
    __anvil_assert_return(sectors == 0 || cursor != nullptr);
    __anvil_assert_return(sectors <= UINT8_MAX);
    ...
    if (region_file->size == 0) {
        ...
        const anvil_result ret = realloc_region_file(region_file, (HEADER_SECTORS + sectors) * SECTOR_SIZE);
        if (ret != ANVIL_OK) {
            return ret;
        }
    ...
    int64_t a_index = 0;
    while (get_sector_count(region_file->file, chunk_indices[a_index]) == 0) a_index++;
    for (int64_t b_index = a_index + 1; b_index < SIZE_X * SIZE_Z; a_index = b_index, b_index++) {
        while (get_sector_count(region_file->file, chunk_indices[b_index]) == 0) b_index++;
        ...
        if (free_count >= sectors) {
            ...
            *cursor = region_file->file + (a_offset + a_count) * SECTOR_SIZE;
            ...
            return realloc_region_file(region_file, get_end_sector(region_file->file, chunk_indices));
        }
    }
...
anvil_result anvil_write(
    ...
    __anvil_assert_return(file != nullptr);
    __anvil_assert_return(in_len == 0 || in != nullptr);
    __anvil_assert_return(file->file != nullptr || file->size == 0);
...
anvil_result anvil_file_close(struct anvil_file *file) {
    ...
    if (
        ...
        __anvil_errno_return(close(file->fd));
    }
