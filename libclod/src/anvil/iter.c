...
int anvil_sort_z_order(
    ...
    const int64_t *const pos1,
    const int64_t *const pos2
) {
    return 0;
}
...
anvil_iter *anvil_iterate_dir(
    ...
    const size_t nchunks = index_size(dir->layout);
    ...
    while ((ent = readdir(d))) {
        ...
        if (
            ...
            for (size_t i = 0; i < nchunks; i++, chunks_count++) {
                coords_from_index(chunks + chunks_count * dir->layout.count, i, region_coords, dir->layout);
            }
        }
    }
    closedir(d);
